...
Y_columns = ['RESU', 'RESUO', 'RESUP', 'RESUI', 'RESUH', 'RESUE', 'RESUC', 'RDS', 'SFT', 'SFTW', 'SUPYN']   # 예시로 사용할 Y 열들

# 학습에 사용한 특성 이름 (X_columns) 순서
X_columns = ['SITEID', 'MAGE', 'GRAN', 'PARN', 'AMNI', 'MULG', 'BIR', 'PREP', 'DM', 
             'HTN', 'CHOR', 'PROM', 'PROM_H', 'STER', 'STERD', 'STERP', 'ATBYN', 
             'DELM', 'BIRYR', 'GAD', 'BWEI']
...
# 입력받을 값들
SITEID = st.number_input("SITEID", min_value=1, max_value=10, value=5)
BIRYR = st.number_input("출생 연도 (BIRYR)", min_value=2000, max_value=2024, value=2024)
GA_w = st.number_input("임신 나이 (주, GA_w)", min_value=20, max_value=45, value=30)
GA_d = st.number_input("임신 나이 (일, GA_d)", min_value=0, max_value=6, value=4)
GAD = (GA_w * 7) + GA_d  # GA_w와 GA_d로 GAD 값 계산
BWEI = st.number_input("출생 체중 (BWEI, g)", min_value=500, max_value=5000, value=3000)
...
STERD = st.selectbox("스테로이드 약제 (STERD)", [0, 1], format_func=lambda x: "없음" if x == 0 else "있음")
...
new_X_data = pd.DataFrame([[
    SITEID, MAGE, GRAN, PARN, AMNI, MULG, BIR, PREP, DM, 
    HTN, CHOR, PROM, PROM_H, STER, STERD, STERP, ATBYN, 
    DELM, BIRYR, GAD, BWEI
]], columns=X_columns)
...
if st.button("결과 예측"):
    ...
    st.header("예측 결과")
    st.dataframe(result_table, height=500, width=800)