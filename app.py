...
inputs = {
    ...
    'amni': st.selectbox("Amniotic Fluid" if lang == 'English' else "양수량", [1, 2, 3, 4],
                         format_func=lambda x: ["Normal", "Oligo", "Poly", "Unknown"] if lang == 'English' else ["정상", "과소", "과다", "모름"][x-1]),
    'mulg': st.selectbox("Multiplicity" if lang == 'English' else "다태 정보", [1, 2, 3, 4],
                         format_func=lambda x: ["Singleton", "Twin", "Triplet", "Quad+"] if lang == 'English' else ["Singleton", "Twin", "Triplet", "Quad 이상"][x-1]),
    'bir': st.selectbox("Birth Order" if lang == 'English' else "출생 순서", [0, 1, 2, 3, 4],
                        format_func=lambda x: ["Single", "1st", "2nd", "3rd", "4th+"] if lang == 'English' else ["단태", "1st", "2nd", "3rd", "4th 이상"][x]),
    'prep': st.selectbox("Pregnancy Type" if lang == 'English' else "임신 과정", [1, 2],
                         format_func=lambda x: ["Natural", "IVF"] if lang == 'English' else ["자연임신", "IVF"][x-1]),
    'dm': st.selectbox("Diabetes" if lang == 'English' else "당뇨", [1, 2, 3],
                       format_func=lambda x: ["None", "GDM", "Overt"] if lang == 'English' else ["없음", "GDM", "Overt DM"][x-1]),
    'htn': st.selectbox("Hypertension" if lang == 'English' else "고혈압", [1, 2, 3],
                        format_func=lambda x: ["None", "PIH", "Chronic"] if lang == 'English' else ["없음", "PIH", "Chronic HTN"][x-1]),
    'chor': st.selectbox("Chorioamnionitis" if lang == 'English' else "융모양막염", [1, 2, 3],
                         format_func=lambda x: ["No", "Yes", "Unknown"] if lang == 'English' else ["없음", "있음", "모름"][x-1]),
    'prom': st.selectbox("PROM" if lang == 'English' else "조기 양막 파열", [1, 2, 3],
                         format_func=lambda x: ["No", "Yes", "Unknown"] if lang == 'English' else ["없음", "있음", "모름"][x-1]),
    'ster': st.selectbox("Steroid Use" if lang == 'English' else "스테로이드 사용", [1, 2, 3],
                         format_func=lambda x: ["No", "Yes", "Unknown"] if lang == 'English' else ["없음", "있음", "모름"][x-1]),
    'sterp': st.selectbox("Steroid Completion" if lang == 'English' else "스테로이드 완료 여부", [0, 1, 2, 3],
                          format_func=lambda x: ["None", "Incomplete", "Complete", "Unknown"] if lang == 'English' else ["미투여", "미완료", "완료", "모름"][x]),
    'sterd': st.selectbox("Steroid Type" if lang == 'English' else "스테로이드 약제", [0, 1, 2, 3, 4],
                          format_func=lambda x: ["None", "Dexa", "Beta", "Dexa+Beta", "Unknown"] if lang == 'English' else ["미투여", "Dexa", "Beta", "Dexa+Beta", "모름"][x]),
    'atbyn': st.selectbox("Antibiotics" if lang == 'English' else "항생제 사용", [1, 2],
                          format_func=lambda x: ["No", "Yes"] if lang == 'English' else ["없음", "있음"][x-1]),
    'delm': st.selectbox("Delivery Mode" if lang == 'English' else "분만 방식", [1, 2],
                         format_func=lambda x: ["Vaginal", "Cesarean"] if lang == 'English' else ["질식분만", "제왕절개"][x-1])
}
